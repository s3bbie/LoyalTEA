...
export default async function handler(req, res) {
  try {
    ...
    const new_members_week = members.filter(
    ...
    // ✅ Fixed: use reward_prices(reward_name, price)
    const { data: redeems, error: redeemErr } = await supabaseAdmin
      .from("redeems")
      .select("user_id, created_at, reward_id, reusable, reward_prices(reward_name, price)");
    ...
    // --- 6️⃣ Build Insights ---
    const total_co2_saved = members
      .reduce((sum, m) => sum + (parseFloat(m.total_co2_saved) || 0), 0)
      .toFixed(2);

    const avg_stamps_per_active =
      active_members > 0
        ? (
            members.reduce((sum, m) => sum + (m.stamp_count || 0), 0) /
            active_members
          ).toFixed(1)
        : 0;

    // ✅ Use reward_prices.reward_name for readable drink names
    const mostRedeemed =
      redeems.length > 0
        ? Object.entries(
            redeems.reduce((acc, r) => {
              const name = r.reward_prices?.reward_name || "Unknown";
    ...
    // ✅ Compute total revenue from reward_prices.price
    const total_revenue = redeems
      .reduce((sum, r) => sum + (parseFloat(r.reward_prices?.price) || 0), 0)
      .toFixed(2);

    const insights = {
      most_redeemed_drink: mostRedeemed,
      avg_stamps_per_active,
